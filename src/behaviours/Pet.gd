...
func entity_interacted(interaction_type: int) -> void:
    ...
    if OS.get_ticks_msec() < last_interaction + cooldown:
        return
...
func emote(index: int) -> void:
    ...
    var origin_position := entity.sprite_emote.position
    ...
    tween.tween_property(entity.sprite_emote, "position:y", origin_position.y + -2.0, 0.25)
    tween.tween_property(entity.sprite_emote, "position:y", origin_position.y, 0.25)
    yield(tween, "finished")
