...
func _process(delta: float):
	...
	if Globals.game_state == GameStates.INTRO:
		if Globals.game_state_entered == false:
			var ui_intro_load = load("res://media/scenes/ui/IntroUI.tscn")
			var intro = ui_intro_load.instance()
			add_child(intro)
			Globals.ui_intro = intro
			Globals.game_state_entered = true
			...
			Globals.world.remove_child(Globals.current_level)
